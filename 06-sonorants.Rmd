...
We can use the following [file with Andic nasals](https://goo.gl/5D9DNR) and the following R code for visualisation:
...
So in order to solve this you need:

* Download the file.
* Open it in Praat.
* Find nasals and create spectral slices.
* Go to Praat Objects window and select your spectral Slice (the name will start with `Spectrum ...`).
* Press `Tabulate` and `Ok`.
* The result (the name started with `Table ...`) can be saved as `comma-separated file`.
* After you saved the files (and uploaded them to rstudio.cloud if you need so) you can use an R scripts below.